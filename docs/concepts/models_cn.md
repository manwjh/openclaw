...
read_when:
  - 添加或修改模型 CLI（models list/set/scan/aliases/fallbacks）时
  - 更改模型回退行为或选择 UX 时
  - 更新模型扫描探测（工具/图像）时
original_path: "docs/concepts/models.md"
...
有关身份验证配置文件轮换、冷却时间以及它们如何与回退交互，请参阅[/concepts/model-failover](/concepts/model-failover)。
...
它可以为常见提供程序设置模型 + 身份验证，包括**OpenAI Code (Codex)
订阅**（OAuth）和**Anthropic**（推荐 API 密钥；也支持 `claude
setup-token`）。
...
- `agents.defaults.models`（允许列表 + 别名 + 提供程序参数）
- `models.providers`（写入 `models.json` 的自定义提供程序）

模型引用被规范化为小写。提供程序别名如 `z.ai/*` 规范化为
`zai/*`。
...
注意事项：
- `/model`（和 `/model list`）是一个紧凑的、编号的选择器（模型系列 + 可用提供程序）。
- `/model <#>` 从该选择器中选择。
- `/model status` 是详细视图（身份验证候选者，以及配置时的提供程序端点 `baseUrl` + `api` 模式）。
...
显示解析的主要模型、回退、图像模型和配置的提供程序的身份验证概述。
它还显示在身份验证存储中找到的配置文件的 OAuth 过期状态（默认在 24 小时内警告）。`--plain` 仅打印
解析的主要模型。
OAuth 状态始终显示（并包含在 `--json` 输出中）。如果配置的
提供程序没有凭据，`models status` 打印**缺少身份验证**部分。
JSON 包括 `auth.oauth`（警告窗口 + 配置文件）和 `auth.providers`
（每个提供程序的有效身份验证）。
使用 `--check` 进行自动化（缺少/过期时退出 `1`，即将过期时退出 `2`）。

首选的 Anthropic 身份验证是 Claude Code CLI setup-token（可在任何地方运行；如果需要，在网关主机上粘贴）：
...
`openclaw models scan` 检查 OpenRouter 的**免费模型目录**，可以
可选地探测模型以获取工具和图像支持。
...
在 TTY 中运行时，您可以交互式地选择回退。在非交互式
模式下，传递 `--yes` 以接受默认值。

## 模型注册表（`models.json`）

`models.providers` 中的自定义提供程序被写入代理目录下的 `models.json`
（默认 `~/.openclaw/agents/<agentId>/models.json`）。除非 `models.mode` 设置为 `replace`，否则此文件
默认合并。
